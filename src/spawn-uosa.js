...
async function getUODir() {
  const uosaRegistryKey =
    "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\GameUX\\Games\\{30E124D7-B515-4778-91C5-4714CB238907}";
  return new Promise((resolve, reject) => {
    regedit.list(uosaRegistryKey, async (err, result) => {
      const uosaDir =
        result[uosaRegistryKey].values.ConfigApplicationPath.value;
      resolve(uosaDir);
    });
  });
}
async function main() {
  ...
  console.log(
    "You can make your own changes to this tool by editing the javascript code in the src/ directory."
  );
  console.log(
    "To update the code, you need to install nodejs and npm and then run 'npm install' followed by 'npm run build'.\n"
  );
